...
class Onshape:
    ...
    Attributes:
        - stack (str): Base URL
        - creds (Union[str, dict], default='./creds.json'): Credentials (location or data)
    ...
    def __init__(self, stack, creds='./creds.json', logging=True):
        ...
        if isinstance(creds, dict) and 'access_key' in creds.keys() and 'secret_key' in creds.keys():
            self._access_key = creds['access_key'].encode('utf-8')
            self._secret_key = creds['secret_key'].encode('utf-8')
        elif isinstance(creds, str) and os.path.isfile(creds):
    ...
    def _make_auth(self, method, date, nonce, path, query={}, ctype='application/json'):
        ...
        if self._logging:
            utils.log({'query': query, 'hmac_str': hmac_str, 'signature': signature, 'auth': auth})
    ...
    def request(self, method, path, query={}, headers={}, body={}, base_url=None, timeout=60):
        """
